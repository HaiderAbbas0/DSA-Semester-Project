...
class Vehicle
{
    ...
    {
        if (path[curIndex] == end)
        {
            cout << id << ": reached destination";
            cout << endl;
            return;
        }
    ...
    void displayAllPaths() 
{
    ...
    for (int i = 0; i < vehicleCount; i++) 
    {
        ...
        VehiclePathNode* current = vehicle->head;

        if (current == nullptr) 
        {
            cout << "No paths available for this vehicle.\n";
        } 
        else 
        {
            while (current != nullptr) 
            {
                cout << "Path: ";
                for (int j = 0; current->path[j] != '\0'; j++) 
                {
                    cout << current->path[j];
                    if (current->path[j + 1] != '\0') 
                    {
                        cout << " -> ";
                    }
                }
                cout << ", Distance: " << current->travelTime << "\n";
                current = current->next;
            }
        }
