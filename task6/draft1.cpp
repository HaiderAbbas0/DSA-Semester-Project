...
class Map
{
    ...
    /* reroute time function, calculates congestion+time for every path */
    /* another reroute function, which calls another function for traversal from our next intersection to the
    end of the path*/
    ...
    void moveVehicle()
    {
        for (int i = 0; i < vehicleCount;)
        {
            ...
            if (congestionLevel > 6) 
            {
                ...
                //reroute(congestionLevel, vehicles[i]);// send target aswell or next intersection
            }
            else
            {
