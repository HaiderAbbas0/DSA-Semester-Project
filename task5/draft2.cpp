   ...
   void route(TrafficNetwork& network)
{
    ...
    if (result.first == INF) {
        ...
        cout << vehicleID << " is taking the route: ";
        cout << path << endl; 
    }
...
class Map
{
    ...
    {
        cout << "Testing emergency routes..." << endl;
        cout << "Total emergency vehicles: " << emergencyvehicleCount << endl; // Debugging line
        for (int i = 0; i < emergencyvehicleCount; ++i) 
        {
            cout << "Routing emergency vehicle: " << emergencyVehicles[i]->getvehicleID() << endl; 
            emergencyVehicles[i]->route(graph);  
            cout << "Finished routing for vehicle: " << emergencyVehicles[i]->getvehicleID() << endl; // Debugging line
        }
